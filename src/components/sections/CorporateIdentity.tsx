...
const VALUES = [
  {
    ...
    text: "Two decades building Mumbai's confidence through ethical redevelopment.",
    delay: 0.1,
  },
  {
    icon: LucideFactory,
    title: "Industrial Vision",
    text: "Expertise in complex industrial park and logistics center development.",
    delay: 0.2,
  },
  {
    icon: LucideLandmark,
    title: "Local Base",
    text: "Rooted in Mumbai, our base ensures hyper-local project execution and insight.",
    delay: 0.3,
  },
  {
    icon: LucideBadgeCheck,
    title: "Quality Assurance",
    text: "Uncompromising standards from blueprint to handover, guaranteed.",
    delay: 0.4,
  },
];
...
export default function CorporateIdentity() {
  ...
  useEffect(() => {
    // 1. Parallax/Zoom Effect for the background element
    gsap.to(parallaxRef.current, {
      scale: 1.1, // Zooms in slowly
      y: 100, // Moves down slightly
      scrollTrigger: {
    ...
    // 2. Staggered Fade-in for the cards
    cardRefs.current.forEach((card, index) => {
      gsap.fromTo(
        ...
        },
        {
          ...
          scrollTrigger: {
            trigger: card,
            start: "top 85%", // Starts when the card is 85% up the screen
    ...
    return () => {
      // Clean up ScrollTriggers on unmount for performance
  ...
  return (
    ...
    >
      ...
      <div
        ref={parallaxRef}
        className="absolute inset-0 z-0 bg-concrete/10 opacity-30 transform scale-100"
        style={{
      ...
      <div className="relative z-10 container mx-auto px-4 text-center">
        <h2 ref={titleRef} className="text-5xl font-serif text-light mb-4">
          Our Foundation. Our Identity.
        </h2>
        <p className="text-concrete text-xl mb-20 max-w-3xl mx-auto">
          We are Archcon. Developers defined by our commitment to quality,
          transformation, and the Mumbai community.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          {VALUES.map((item, index) => (
            <div
              ...
              className="bg-primary/70 backdrop-blur-sm p-8 rounded-xl border border-accent/10 shadow-xl text-left transform translate-y-0 opacity-0"
            >
              <item.icon className="h-10 w-10 text-accent mb-4" />
              <h3 className="text-2xl font-semibold text-light mb-3">
                {item.title}
              </h3>
              <p className="text-concrete">{item.text}</p>
            </div>
          ))}
        </div>
      </div>
